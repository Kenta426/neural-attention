...
class BaselineLSTM(object):
    ...
    def __init__(self):
        ...
        with tf.variable_scope('output'):
            self.output = tf.placeholder(tf.int32, [BATCH], name = 'one_hot')
        ...
        with tf.variable_scope('soft_max'):
            units_in = self.hypothesis_embed.get_shape().as_list()[-1]
            ...
            tf.add_to_collection(tf.GraphKeys.REGULARIZATION_LOSSES, tf.nn.l2_loss(weights))
            self.logits = tf.nn.xw_plus_b(self.hypothesis_embed, weights, biases)
